<script lang="ts">
	...
	import { supabase, getTasks, getEntries, logEntry, deleteEntry } from '$lib/supabase';
	import { DEFAULT_TASKS } from '$lib/tasks';
	import { FAMILY_MEMBERS } from '$lib/members';

	let tasks = $state(DEFAULT_TASKS);
	let entries = $state<any[]>([]);
	let selectedMember = $state(FAMILY_MEMBERS[0].id);
	...
	// Timer state
	let activeTimer = $state<{ taskId: string; taskName: string; taskEmoji: string; productive: boolean; startTime: number } | null>(null);
	...
	async function loadedToday() {
		isLoading = true;
		try {
			[ tasks ] = await Promise.all([ getTasks() ]);
			const memberName = FAMILY_MEMBERS.find((m) => m.id === selectedMember)?.name ?? 'Someone';
			entries = await getEntries(memberName, today);
		} catch {
			tasks = DEFAULT_TASKS;
			entries = [];
		}
	...
	async function handleQuickSave() {
		...
		const minutes = parseInt(quickMinutes as unknown as string) || 15;
		const task = tasks.find(t => t.name.toLowerCase() === quickTask.trim().toLowerCase()) ?? tasks[0];
		const entry = {
			member_name: FAMILY_MEMBERS.find(m => m.id === selectedMember)?.name ?? 'Someone',
			task_name: task.name,
...
<div class="app">
	...
	{:else if tasks.length === 0}
		<div class="empty">No tasks set up yet. Add tasks in your Supabase dashboard.</div>
	{:else}
		<div class="main-grid">
				...
				<div class="tasks-list">
					{#each tasks as task}
						<button
							...
							onclick={() => activeTimer?.taskId === task.id ? stopTimer() : startTimer(task)}
						>
			...
			<section class="summary-panel">
				...
				<div class="summary-cards">
					<div class="summary-card good">
						...
						<span class="sum-value">{new Date(0, 0, 0, 0, productiveMinutes).toISOString().slice(14, 19)}</span>
						<span class="sum-sub">{ productiveMinutes > 0 ? Math.round(productiveMinutes / totalMinutes * 100) : 0 }% of today</span>
					</div>
					<div class="summary-card neutral">
						<span class="sum-emoji">⏸</span>
						<span class="sum-label">Screen / Leisure</span>
						<span class="sum-value">{new Date(0, 0, 0, 0, nonProductiveMinutes).toISOString().slice(14, 19)}</span>
						<span class="sum-sub">{ nonProductiveMinutes > 0 ? Math.round(nonProductiveMinutes / totalMinutes * 100) : 0 }% of today</span>
					</div>
					<div class="summary-card total">
						<span class="sum-emoji">⏱</span>
						<span class="sum-label">Total</span>
						<span class="sum-value">{new Date(0, 0, 0, 0, totalMinutes).toISOString().slice(14, 19)}</span>
					</div>
				</div>
					...
					{:else}
						{#each entries as entry}
							<div class="entry-row" style:border-left-color={entry.productive ? '#22C55E' : '#EF4444'}>
								...
								{/if}
								<button class="del-btn" onclick={() => handleDelete(entry.id)} title="Delete">✕</button>
							</div>
						{/each}
					{/if}
				</div>
...
<style>
	...
	.timer-active.needs-restart { border-color: #7C3AED; }
